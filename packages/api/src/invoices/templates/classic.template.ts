...
export const classicTemplate: TemplateFunction = (doc, invoice, company): void => {
  ...
  doc.moveDown(0.5)
  doc.font('Helvetica-Bold').text(`Subtotal: ${fmt(invoice.subtotal)}`, { align: 'right' })
  ...
  if (invoice.extraItems.length > 0) {
    ...
    doc.moveDown(0.5)
    doc.font('Helvetica-Bold').text(`Extras: ${fmt(invoice.extrasTotal)}`, { align: 'right' })
  }
